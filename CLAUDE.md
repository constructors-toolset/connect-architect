...
| 산출물 종류 | 위치 |
|---|---|
| 서브 프로젝트 저장소 (git 서브모듈) | `repo/<프로젝트-이름>/` + `repo/README.md` 목록 갱신 |
...
- 서브 프로젝트는 별도 저장소로 만들고 `repo/<프로젝트-이름>/`에 **git 서브모듈**로 연결합니다 (`git submodule add <url> repo/<이름>`).
- 서브 프로젝트의 코드·문서는 그 저장소에서 수정합니다. 허브에서는 서브모듈이 가리키는 커밋만 갱신합니다.
- 서브 프로젝트를 연결하거나 상태가 바뀌면 **반드시** `repo/README.md`의 목록 표(상태, 최종 갱신일)와 "진행 기록"을 함께 갱신합니다. 진행 기록은 덮어쓰지 않고 날짜별로 누적합니다.
